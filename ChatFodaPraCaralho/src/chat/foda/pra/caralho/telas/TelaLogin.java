...
public class TelaLogin extends JFrame {
	...
	private JPanel getMainPanel() {
		...
		pnlMain.add(getLogin(),
		        new GridConstraints().setInsets(5).setAnchor(GridConstraints.WEST).setFill(GridBagConstraints.HORIZONTAL)
		                .setOccupiedSize(GridConstraints.REMAINDER, 1).setGridWeight(1, 0));
		
		jlbRegistrar = new JLabel("Ainda não possui uma conta?");
		pnlMain.add(jlbRegistrar,
		        new GridConstraints().setAnchor(GridConstraints.EAST).setInsets(5).setOccupiedSize(GridConstraints.RELATIVE, 1)
		                .setGridWeight(1, 0));
	...
	private JPanel getLogin() {
		...
		pnlLogin.add(jtfId, new GridConstraints().setInsets(5).setAnchor(GridConstraints.WEST)
		        .setFill(GridConstraints.HORIZONTAL).setOccupiedSize(GridConstraints.REMAINDER, 1).setGridWeight(1, 0));
		
		jlbSenha = new JLabel("Senha:");
		pnlLogin.add(jlbSenha, new GridConstraints().setAnchor(GridConstraints.EAST).setInsets(5));
		
		jpfSenha = new JPasswordField();
		pnlLogin.add(jpfSenha,
		        new GridConstraints().setInsets(5).setAnchor(GridConstraints.WEST).setFill(GridConstraints.HORIZONTAL)
		                .setOccupiedSize(GridConstraints.REMAINDER, 1).setGridWeight(1, 0));
		
		jbtConfirmar = new JButton("Confirmar");
		pnlLogin.add(jbtConfirmar,
		        new GridConstraints().setInsets(5).setAnchor(GridConstraints.EAST).setOccupiedSize(GridConstraints.REMAINDER, 1));
	...
	public void actions() {
		...
		jbtConfirmar.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent arg0) {
				String email = jtfId.getText();
				String senha = new String(jpfSenha.getPassword());
				
				if (!email.isEmpty() && !senha.isEmpty()) {
					try {
						UsuarioLogado usuarioLogado = cliente.getService().login(cliente.getClienteService(), email, senha);
